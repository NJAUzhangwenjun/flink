 ...
 * @date 2021/1/273:44
 */
public class WithTimeStampWithLateDateNoWatermarkSource extends RichSourceFunction<NCTimeMessage> {
    ...
    public void run(SourceContext<NCTimeMessage> sourceContext) throws Exception {
        ...
        while (!isCancel) {
            ...
            if (message.getMessage().contains("RichSourc")) {
                message.setTimeStamp(message.getTimeStamp()-4000L);
            }
            synchronized (this) {
